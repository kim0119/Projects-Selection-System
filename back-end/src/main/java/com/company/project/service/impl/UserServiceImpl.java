...
public class UserServiceImpl extends AbstractService<User> implements UserService {
    ...
    public Result logout(Long userId) {
        ...
        return ResultGenerator.genFailResult(ResultCode.NOT_LOGIN_EXCEPTION,"User not logged in");
    }
    ...
    public Result login(LoginVo vo) {
        ...
        //根据用户名查询
        user = userMapper.findUserByUserName(vo.getUserName(),null);//数据库中查找信息是否存在

        //判断用户密码是否正确
        if (null == user){
            return ResultGenerator.genFailResult(ResultCode.USER_NOT_EXIST,"User information does not exist [account may be deactivated or deleted]");
        }

        if(!vo.getPassword().equals(user.getPassword())){
            return ResultGenerator.genFailResult(ResultCode.PASSWORD_ERROR,"Password input error, please re-enter");
        }

        //获取对应的模块
        //拿到了此用户能够进行的操作，通过此来渲染对应的首页
    ...
    public Result updateUser(User user) {
        ...
        //用户名不可重复
        if (null != newUser){
            return ResultGenerator.genFailResult(ResultCode.USER_ALREADY_EXIST,"Username already exists, please login");
        }
